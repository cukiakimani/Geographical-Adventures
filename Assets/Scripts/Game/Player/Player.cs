...
public class Player : NetworkBehaviour
{
    ...
    public override void Spawned()
    {
        ...
        if (Object.InputAuthority == Runner.LocalPlayer)
        {
            ...
            FindObjectOfType<PlayerInputHandler>().player = this;
        }
    ...
    {
        if (GetInput(out PlaneInput i))
        {
            // Turning
            turnInput = i.moveInput.x;
            pitchInput = i.moveInput.y;

            // Speed
            baseTargetSpeed += (maxSpeed - minSpeed) / accelerateDuration * i.accelerateDir * Runner.DeltaTime;
        ...
        UpdateBoostTimer(Runner.DeltaTime);

        UpdateGraphics(Runner.DeltaTime);
        UpdateDevMode();

        gameCamera.ManualUpdate();
    }
    ...
    public void UpdateMovementInput(Vector2 moveInput, float accelerateDir, bool boosting)
    {
        ...
        input.accelerateDir = accelerateDir;
    ...
    void HandleMovement(float deltaTime)
    {
        ...
        float turnAmount = smoothedTurnSpeed * deltaTime;
        totalTurnAngle += turnAmount;


        // Update speed
        float targetSpeed = (boosting) ? boostSpeed : baseTargetSpeed;
        currentSpeed = Mathf.Lerp(currentSpeed, targetSpeed, 1 - Mathf.Pow(speedSmoothing, deltaTime));
        ...
        // Update elevation
        currentElevation += verticalVelocity * deltaTime;
        ...
        UpdatePosition(forwardSpeed, deltaTime);
    ...
    void UpdateBoostTimer(float deltaTime)
    {
        // Decrease boost timer when boosting
        if (boosting)
        {
            boostTimeRemaining = Mathf.Max(0, boostTimeRemaining - deltaTime);
        }
        // Increase boost timer gradually when time has been gained
        if (boostTimeToAdd > 0)
        {
            const float boostAddSpeed = 4;
            float boostTimeToAddThisFrame = Mathf.Min(deltaTime * boostAddSpeed, boostTimeToAdd);
    ...
    void UpdatePosition(float forwardSpeed, float deltaTime)
    {
        // Update position
        Vector3 newPos = transform.position + transform.forward * forwardSpeed * deltaTime;
        if (worldIsSpherical)
        {
    ...
    void UpdateGraphics(float deltaTime)
    {
        ...
        // Rotate propeller
        propeller.localEulerAngles += Vector3.forward * propellerSpeed * deltaTime;
        ...
        SetNavigationLightScale((navigationLightsOn) ? 1 : 0, true, deltaTime);

        void UpdateAileronGraphic(Transform aileron, float targetAngle)
        {

            Vector3 rot = aileron.localEulerAngles;
            float smoothAngle = Mathf.LerpAngle(rot.x, targetAngle, deltaTime * 5);
    ...
    void SetNavigationLightScale(float scale, bool smooth = false, float deltaTime = 0f)
    {
        for (int i = 0; i < navigationLights.Length; i++)
        {
            if (smooth)
            {
                ...
                navigationLights[i].localScale = Vector3.one * Mathf.Lerp(currentScale, scale, deltaTime);
            }
            else
            {
